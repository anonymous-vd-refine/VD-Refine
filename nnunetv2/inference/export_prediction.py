...
def convert_predicted_logits_to_segmentation_with_correct_shape(predicted_logits: Union[torch.Tensor, np.ndarray],
    ...
    foreground_threshold_raw = os.environ.get('NNUNET_BINARY_FOREGROUND_THRESHOLD')
    if foreground_threshold_raw is None:
        segmentation = label_manager.convert_probabilities_to_segmentation(predicted_probabilities)
    else:
        foreground_threshold = float(foreground_threshold_raw)
        if not 0 < foreground_threshold < 1:
            raise ValueError(
                'NNUNET_BINARY_FOREGROUND_THRESHOLD must be strictly between 0 and 1, '
                f'got {foreground_threshold}'
            )
        if label_manager.has_regions or label_manager.num_segmentation_heads != 2 or \
                tuple(label_manager.foreground_labels) != (1,):
            raise RuntimeError(
                'NNUNET_BINARY_FOREGROUND_THRESHOLD is only supported for a non-region binary '
                'segmentation dataset with foreground label 1'
            )
        # The ISIC calibration is performed on the exported foreground probability after
        # resampling. Apply the selected threshold at exactly that same point in the standard
        # export path. Background remains 0 and foreground remains label 1.
        segmentation = (predicted_probabilities[1] >= foreground_threshold).to(torch.uint8)
